...
const CACHE_KEY = 'studyneo_forum_cache'
const LIMIT_KEY = 'studyneo_forum_limit'
const MAX_PER_DAY = 1
const BAD_WORDS = ['puta', 'mierda', 'coño', 'joder', 'cabron', 'pendejo', 'verga', 'puto', 'zorra', 'imbecil', 'idiota', 'estupido', 'maricon', 'marica', 'putita', 'putito', 'sorro', 'zorro']

function getFingerprint() {
  const key = 'studyneo_anon_fp'
...
function saveCache(msgs) {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify(msgs.slice(0, 50))) } catch {}
}
...
export default function FeedbackForum({ onBack, lang = 'es' }) {
  ...
  const [fbDown,     setFbDown]     = useState(false)
  ...
  useEffect(() => {
    ...
    if (stored.date !== todayStr) {
      setMsgLimit(0)
      localStorage.setItem(LIMIT_KEY, JSON.stringify({ date: todayStr, count: 0 }))
    } else {
    ...
    const unsub = onValue(
      ...
      snap => {
        const data = snap.val()
        const arr = data
          ? Object.values(data).sort((a, b) => b.timestamp - a.timestamp).slice(0, 50)
          : []
        setMessages(arr)
        saveCache(arr)
        setFbDown(false)
  ...
  const handlePost = () => {
    if (!text.trim()) return
    if (fbDown) { setError('El foro no está disponible en este momento. Inténtalo más tarde.'); return }
    ...
    if (text.length > 500) { setError('El mensaje es muy largo (máximo 500 caracteres).'); return }
    if (msgLimit >= MAX_PER_DAY) {
      setError(`Límite diario alcanzado. Vuelve mañana.`)
      return
    }
    if (!agreed) { setError('Debes aceptar las reglas del foro antes de publicar.'); return }
    ...
    push(ref(db, 'studyneo/forum_posts'), newMsg).catch(() => {
      setFbDown(true)
      setError('No se pudo publicar. Firebase no disponible.')
    })

    const newCount = msgLimit + 1
    localStorage.setItem(LIMIT_KEY, JSON.stringify({ date: new Date().toISOString().slice(0, 10), count: newCount }))
    setMsgLimit(newCount)
    setText('')
    setError('')
  }

  const isDisabled = msgLimit >= MAX_PER_DAY || fbDown
      ...
      <div className="todo-card" style={{ padding: '20px', margin: '0 20px 30px', background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: 'var(--text-2)', cursor: 'pointer', paddingLeft: '4px' }}>
            <input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} style={{ accentColor: 'var(--accent)' }} disabled={fbDown} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
            ...
            <button
              ...
              style={{ maxWidth: '160px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', padding: '10px 16px', background: isDisabled ? 'var(--bg-input)' : 'var(--accent)', color: isDisabled ? 'var(--text-4)' : '#fff', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: isDisabled ? 'not-allowed' : 'pointer' }}
            >
